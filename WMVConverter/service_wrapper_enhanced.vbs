' Enhanced WMV Converter Service Wrapper
' Based on the working service_wrapper.vbs with improvements
...
ConfigPath = ScriptDir & "\watchdog_config.ini"
...
LogFile = LogPath & "\service_wrapper_" & Year(Now) & Right("0" & Month(Now), 2) & Right("0" & Day(Now), 2) & "_" & Right("0" & Hour(Now), 2) & Right("0" & Minute(Now), 2) & Right("0" & Second(Now), 2) & ".log"
...
' Start logging
WriteLog "=== Enhanced WMV Converter Service Wrapper Started ==="
...
If Not FSO.FileExists(ConfigPath) Then
    WriteLog "WARNING: Config file not found, using defaults: " & ConfigPath
End If
...
' Run the converter
WriteLog "Starting enhanced WMV converter..."
